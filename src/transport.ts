...
/** The shape this module reads and writes on the global. */
 ...
 */
function withDeadline(signal: AbortSignal | null | undefined, timeoutMs: number): AbortSignal {
  const timeout = AbortSignal.timeout(timeoutMs)
  if (signal === null || signal === undefined) return timeout
 ...
 */
export function installRequestDeadline(timeoutMs: number): () => void {
  const effective = Math.max(timeoutMs, MIN_REQUEST_TIMEOUT_MS)
  const scope = globalThis as unknown as Record<string, TransportGlobal | undefined>
  ...
  const base: typeof globalThis.fetch =
    inner === undefined ? (input, init) => globalThis.fetch(input, init) : inner

  const guarded: typeof globalThis.fetch = (input, init) => {
    const options: RequestInit = init === undefined ? {} : init
  ...
  return () => {
    ...
    if (current !== undefined && current.fetch === guarded) {
      if (previous === undefined) delete scope[TRANSPORT_KEY]
      else scope[TRANSPORT_KEY] = previous
    }
  }
}
