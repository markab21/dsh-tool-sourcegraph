...
import { resolveFieldAlias, FilterType } from './filters.js';
...
// - `foo)` does not succeed, it is not balanced
const scanBalancedParens = (input) => {
    let adjustedStart = 0;
    let balanced = 0;
    let current = '';
    const result = [];
 ...
 */
export const scanPredicate = (field, value) => {
...
export const predicateCompletion = (field) => {
    if (field === 'repo') {
        return [
            {
                label: 'has.path(...)',
        ...
        return [
            {
                ...
                asSnippet: true,
                description: 'Search repos at a specific time in history. Optionally, a base revision can be specified as a second parameter like rev:at.time(yesterday, my-branch)',
            },
        ];
    }
    return [];
